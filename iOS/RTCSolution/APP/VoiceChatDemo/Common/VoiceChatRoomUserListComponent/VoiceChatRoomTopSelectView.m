...
- (void)addConstraints {
    ...
    [self.redImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.appleButton.titleLabel.mas_right).offset(0);
    }];
}
...
- (BaseButton *)onlineButton {
    if (!_onlineButton) {
        ...
        [_onlineButton setTitle:LocalizedString(@"online_audience") forState:UIControlStateNormal];
...
- (BaseButton *)appleButton {
    if (!_appleButton) {
        ...
        [_appleButton setTitle:LocalizedString(@"application_message") forState:UIControlStateNormal];
